...
from app.domain.entities import Track, User, Playlist, RecommendationResult
...
class TestUserEntity:
    ...
    def test_user_missing_id_fails(self):
...
class TestRecommendationEntity:
    def test_recommendation_top_n(self):
        tracks = [
            Track(f"id_{i}", f"Song {i}", "Artist", 100, 50) 
            for i in range(5)
        ]
    ...
    def test_invalid_confidence_score(self):
        with pytest.raises(ValueError, match="confidence_score must be between 0 and 1"):
            RecommendationResult("Sad", [Track("1", "S", "A", 1, 1)], confidence_score=1.5)